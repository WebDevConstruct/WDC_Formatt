...
const [index, setIndex] = useState(0);
  ...
  const {user}= useUser();
  useEffect(() => {
    if(ADMIN_PROMPTS?.length <= 1) return;
...
//if(!isSignedIn) console.log("User not Signed In")
  ...
  return (
// NEW DESIGN
<div className="min-h-screen bg-[#F2F0E9] text-[#483C32] selection:bg-[#483C32] selection:text-[#D4AF37] overflow-x-hidden">
      {/* Decorative Brand Gradient - Now Majestic Brown to Gold */}
      <div className="fixed top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#483C32] via-[#2C2520] to-[#D4AF37]" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-10 space-y-10">
        ...
        <div className="space-y-6">
          <header className="space-y-2">
            <h1 className="text-4xl md:text-6xl font-serif font-black tracking-tight text-[#483C32]">
              Welcome, <span className="text-[#D4AF37] italic">Scholar {username ? username?.toUpperCase() : "User"}</span>
            </h1>
          ...
          {/* THE "FISCHER" UNROLL ANIMATION - Deep Obsidian Brown */}
          <div className="relative h-16 w-full bg-[#2C2520] rounded-2xl flex items-center px-6 overflow-hidden shadow-2xl border border-[#D4AF37]/10">
  <AnimatePresence mode="wait" >
    <motion.div
      // CRITICAL: The key must change for the exit animation to trigger
      key={`prompt-${index}`} 
      initial={{ y: 20, opacity: 0 }} // Changed to 'y' for a more professional "roll" effect
      animate={{ y: 0, opacity: 1 }}
      exit={{ y: -20, opacity: 0 }}
      transition={{ 
        duration: 0.6, 
        ease: [0.16, 1, 0.3, 1] // Custom "Majestic" ease-out
      }}
      className="flex items-center gap-4 text-[#F2F0E9] italic font-medium w-full"
    >
      {/* The Gold Terminal Pointer */}
      <span className="text-[#D4AF37] font-black not-italic text-lg tracking-tighter">

      </span>
      
      <span className="truncate text-sm md:text-lg opacity-90 tracking-tight">
        {ADMIN_PROMPTS[index] || "Loading Insights...."}
      </span>
    </motion.div>
  </AnimatePresence>
  
  {/* The Majestic Gradient Fade to protect text truncation */}
  <div className="absolute right-0 top-0 h-full w-32 bg-gradient-to-l from-[#2C2520]
   via-[#2C2520]/80 to-transparent pointer-events-none" />
</div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {/* Files Card */}
            <Link href="/dashboard/Files" className="block group">
              <Card className="relative overflow-hidden bg-[#F2F0E9] border-2 border-[#483C32] p-1 
              shadow-[8px_8px_0px_0px_rgba(72,60,50,1)] transition-all group-hover:shadow-[12px_12px_0px_0px_rgba(212,175,55,1)] group-hover:border-[#D4AF37] group-hover:-translate-x-1 group-hover:-translate-y-1">
                <div className="bg-[#483C32]/5 p-8 md:p-12 space-y-8">
                  <div className="flex justify-between items-start">
                    <div className="p-4 bg-[#483C32] text-[#D4AF37] rounded-2xl shadow-xl">
                      <Zap className="w-8 h-8" />
                    </div>
                    <div className="px-4 py-2 bg-[#D4AF37] text-[#483C32] rounded-full text-[10px] font-black flex items-center gap-2">
                  ...
                  <div className="pt-8 flex items-center gap-4">
                    <button className="bg-[#483C32] text-[#F2F0E9] px-8 py-4 rounded-xl font-black uppercase text-sm tracking-widest shadow-lg hover:bg-[#2C2520] transition-colors">
                      Initialize Build
                    </button>
                    <span className="text-[#D4AF37] font-black text-xs hidden sm:inline tracking-widest">AUTHENTICATING...</span>
                  </div>
                </div>
              </Card>
            </Link>
...
<Link href="/dashboard/Slides" className="block group">
  <Card className="relative overflow-hidden bg-[#F2F0E9] border-2 border-[#483C32]/40 p-1 shadow-[8px_8px_0px_0px_rgba(72,60,50,0.2)] transition-all group-hover:shadow-[12px_12px_0px_0px_rgba(72,60,50,0.4)] group-hover:border-[#483C32] group-hover:-translate-x-1 group-hover:-translate-y-1">
    <div className="bg-[#483C32]/5 p-8 md:p-12 space-y-8">
      <div className="flex justify-between items-start">
        <div className="p-4 bg-[#483C32]/20 text-[#483C32] rounded-2xl">
          {/* Swapped Monitor for Presentation to match the "Slides" intent */}
          <Presentation className="w-8 h-8" /> 
        </div>
      </div>
      <div className="space-y-4">
        <h2 className="text-4xl md:text-5xl font-serif font-black text-[#483C32]/40 leading-none uppercase italic">Slides</h2>
        <p className="text-[#483C32]/50 text-lg max-w-xl font-medium leading-relaxed">
          The presentation architectural logic is currently under refinement. 
          Standard WDC formatting protocols apply.
        </p>
      </div>
    </div>
  </Card>
          ...
          {/* RIGHT: Dynamic History */}
          <div className="lg:col-span-4 space-y-6">
            <h3 className="text-xs font-black uppercase tracking-[0.3em] text-[#483C32]/60">Majestic Vault</h3>
            <div className="bg-[#483C32]/5 rounded-3xl border border-[#483C32]/10 p-6 flex flex-col min-h-[400px] backdrop-blur-md">
              {pdfHistory.length > 0 ? (
                <div className="space-y-3">
                  {pdfHistory.map((pdf, i) => (
                    <motion.div 
                      whileHover={{ x: 5 }}
                      key={i} 
                      className="flex items-center gap-4 p-4 bg-white/40 rounded-2xl border border-[#483C32]/5 hover:border-[#D4AF37]/40 cursor-pointer"
                    >
                      <div className="bg-[#483C32] p-2 rounded-lg text-[#D4AF37]">
                        <FileText className="w-5 h-5" />
                      </div>
                      <div className="overflow-hidden">
                        <p className="text-sm font-black text-[#483C32] truncate">{pdf.name}</p>
                        <p className="text-[10px] uppercase font-bold text-[#483C32]/40">{pdf.date}</p>
                      </div>
                    </motion.div>
                  ))}
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                  <div className="w-20 h-20 rounded-full border-2 border-dashed border-[#483C32]/20 flex items-center justify-center text-[#483C32]/20 mb-4">
                    <History className="w-10 h-10" />
                  </div>
                  <h4 className="text-xl font-serif font-black text-[#483C32]/20 italic">NO RECORDS</h4>
                </div>
              )}
              
              <Link href="/archive" className="mt-auto pt-6 text-center text-xs font-black text-[#D4AF37] hover:tracking-[0.2em] transition-all uppercase">
                Open Full Vault →
              </Link>
            </div>
          </div>
        ...
        <footer className="pt-16 flex flex-col md:flex-row justify-between gap-8 opacity-60 border-t border-[#483C32]/10">
          <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-[#483C32]">
            <span>SECURE ACCESS v1.0.4</span>
            <span className="w-1 h-1 rounded-full bg-[#D4AF37]" />
         
          </div>
          <div className="flex gap-6 text-[10px] font-black uppercase tracking-widest text-[#483C32]">
            <span className="text-[#D4AF37]">Active: 30-Day Beta Cycle</span>
  ...
  //           <Link href="/dashboard/Files" className="block group">
  //             <Card className="relative overflow-hidden bg-[#F5F5DC] border-2 border-[#5C4033] p-1 
  //             shadow-[8px_8px_0px_0px_rgba(92,64,51,1)] transition-all group-hover:shadow-[12px_12px_0px_0px_rgba(139,0,0,1)] group-hover:border-[#8B0000] group-hover:-translate-x-1 group-hover:-translate-y-1">
  ...
  //                 <div className="space-y-4">
  //                   <h2 className="text-4xl md:text-5xl font-black text-[#5C4033] leading-none uppercase italic">Files</h2>
  //                   <p className="text-[#A52A2A] text-lg max-w-xl font-medium leading-relaxed">
  //                     This module is currently the only operational sequence active for the 30-day testing window. 
  //                     Ensure you are in a controlled environment before initializing.
  //                   </p>
  //                 </div>
